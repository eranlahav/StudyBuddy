...
must_haves:
  truths:
    ...
    - "Four engagement levels exist: high, medium, low, avoidance"
    - "Low engagement/avoidance adjusts mastery confidence downward"
    - "Hebrew reasoning strings explain the engagement assessment"
  artifacts:
    - path: "lib/engagementDetector.ts"
      provides: "Engagement pattern analysis from session metrics"
      exports: ["analyzeEngagement", "ENGAGEMENT_CONFIG"]
  key_links:
    - from: "lib/engagementDetector.ts"
      to: "types.ts"
      via: "EngagementMetrics, EngagementSignal types"
      pattern: "import.*EngagementMetrics.*from"
---

<objective>
Create engagement detection module for analyzing session behavior patterns.

Purpose: Enables the profile to incorporate indirect learning signals from quiz session behavior - detecting avoidance, rushing, incomplete sessions, and adjusting mastery confidence accordingly.

Output: `lib/engagementDetector.ts` with engagement analysis logic
</objective>
...
# Types (Signal types from 04-01)
@types.ts

# Existing fatigue/frustration patterns
...
<task type="auto">
  <name>Task 1: Create Engagement Detector Module</name>
  <files>lib/engagementDetector.ts</files>
  <action>
Create new file `lib/engagementDetector.ts`:
...
```
  ...
  <done>lib/engagementDetector.ts exports analyzeEngagement, buildEngagementMetrics, getEngagementLabel, getEngagementColorClass, ENGAGEMENT_CONFIG</done>
</task>

<task type="auto">
  <name>Task 2: Export from lib/index.ts</name>
  <files>lib/index.ts</files>
  <action>
Add exports for the engagement detector module after the Signal Weights exports:
...
```
  ...
  <verify>Run `npx tsc --noEmit` - should compile without errors</verify>
  <done>Engagement detector functions and config exported from lib/index.ts</done>
</task>
...
2. Engagement detector correctly imports types from types.ts
3. All engagement analysis functions exported from lib/index.ts
4. Hebrew reasoning strings are correct
</verification>

<success_criteria>
- lib/engagementDetector.ts provides analyzeEngagement function
- Four engagement levels correctly detected based on metrics
- Hebrew reasoning strings explain assessment
- Mastery adjustments: high=0, medium=0, low=-0.05, avoidance=-0.10
- TypeScript compiles without errors
</success_criteria>

<output>
After completion, create `.planning/phases/04-multi-signal-integration/04-02-SUMMARY.md`
</output>
