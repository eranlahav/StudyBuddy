...
<task type="auto">
  <name>Task 4A: Create RecommendationsPanel component</name>
  <files>pages/ChildDetails/RecommendationsPanel.tsx</files>
  <action>
Create `pages/ChildDetails/RecommendationsPanel.tsx`:
...
Export navigation helper for onStartQuiz:
```typescript
...
```
  ...
  <verify>Run `npx tsc --noEmit` - component compiles without errors</verify>
  <done>RecommendationsPanel renders recommendations list with goals section and override modal</done>
</task>

<task type="auto">
  <name>Task 4B: Integrate RecommendationsPanel into ChildDetails</name>
  <files>pages/ChildDetails/index.tsx</files>
  <action>
Integrate RecommendationsPanel into the ChildDetails page:
...
2. **Find integration point**:
   - Locate the PlanTab section or appropriate location in the tabs
...
4. **Ensure conditional rendering**:
   - Show when child and selectedSubject are defined
   - Component handles its own loading/empty states
  </action>
  <verify>Run `npm run dev`, navigate to /parent/child/:id, select a subject, verify RecommendationsPanel appears</verify>
  <done>RecommendationsPanel integrated and visible in ChildDetails when subject is selected</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Complete recommendation UI: RecommendationCard with expandable reasoning, OverrideModal with structured rationale, GoalForm for learning goals, and RecommendationsPanel integrated into ChildDetails</what-built>
  <how-to-verify>
1. Start dev server: `npm run dev`
2. Navigate to /parent (login if needed)
3. Click on a child to view ChildDetails
4. Select a subject from the dropdown
5. Verify RecommendationsPanel appears with:
   - 3-5 recommended topics (or empty state if insufficient data)
   - Priority badges (color-coded)
   - Confidence indicators
6. Click "למה?" on a recommendation - verify reasoning expands
7. Click "דחה" - verify OverrideModal opens with radio options
8. Test GoalForm: add a learning goal, verify it appears in the list
9. Verify RTL layout and Hebrew text throughout
  </how-to-verify>
