...
Add UpcomingTest.topic type note: The topic field is a comma-separated string (e.g., "fractions,decimals") per existing schema.
  </action>
...
2. **calculateUrgencyScore(topic, upcomingTests)** - Test proximity:
   - Filter tests where topic appears in test.topic (split by comma)
