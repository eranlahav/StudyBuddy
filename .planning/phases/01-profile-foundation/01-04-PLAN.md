...
must_haves:
  ...
  key_links:
    - from: "pages/ChildDetails/index.tsx"
      ...
      via: "useLearnerProfile hook call"
      pattern: "useLearnerProfile\\(child\\.id"
    - from: "pages/ChildDetails/index.tsx"
...
const {
  ...
  getConfidenceLevel
} = useLearnerProfile(child?.id, childSessions);
```

Note: The hook needs to be called after childSessions is defined but we need child?.id which requires child. The hook handles undefined childId gracefully, so we can call it early and pass child?.id.
...
Important: The hook must be called unconditionally at the top level (React rules of hooks). Call it with child?.id which returns undefined if child is null, and the hook handles that case.
...
3. Hook is called with child?.id and childSessions
4. profile, profileLoading, getConfidenceLevel are passed to AnalysisTab
  </verify>
  <done>ChildDetails calls useLearnerProfile and passes profile/profileLoading/profileConfidence to AnalysisTab</done>
</task>
...
3. ChildDetails calls useLearnerProfile(child?.id, childSessions)
