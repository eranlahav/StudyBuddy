...
must_haves:
  truths:
    ...
    - "Profile bootstraps from existing quiz sessions for children with history"
  artifacts:
    - path: "store.tsx"
  ...
  key_links:
    ...
    - from: "hooks/useLearnerProfile.ts"
      to: "services/profileService.ts"
      via: "bootstrapProfile for existing children"
      pattern: "bootstrapProfile\\(child\\.id,\\s*child\\.familyId"
---

<objective>
Integrate profile updates into the store's addSession flow and create a hook for components to access profile data.

Purpose: This connects the profile backend to React - store.tsx triggers profile updates on quiz completion, while useLearnerProfile hook enables UI components to display profile data with real-time updates. The hook also auto-bootstraps profiles for existing children who have quiz history but no profile yet.
...
<task type="auto">
  <name>Task 2: Create useLearnerProfile Hook with Auto-Bootstrap</name>
  <files>hooks/useLearnerProfile.ts</files>
  <action>
Create new file hooks/useLearnerProfile.ts for profile state management with auto-bootstrap for existing children.
...
```
  ...
  <done>hooks/useLearnerProfile.ts exists with useLearnerProfile hook accepting child: ChildProfile | undefined, auto-bootstrap logic with all 4 params, and getConfidenceMessage utility</done>
</task>
...
5. Hook accepts `child: ChildProfile | undefined` as first parameter
6. Hook handles subscription cleanup on unmount
7. Hook auto-bootstraps profile when profile is null but sessions exist
8. Bootstrap call uses all 4 params: `bootstrapProfile(child.id, child.familyId, sessions, child.grade)`
9. Bootstrap is fire-and-forget (doesn't block UI)
10. Bootstrap only attempted once per child (tracked via ref)
</verification>
...
- useLearnerProfile accepts child object (not just childId) to enable access to familyId and grade
...
- Existing children with sessions get profile bootstrapped automatically on first view
- Bootstrap call includes all required params (id, familyId, sessions, grade)
</success_criteria>
