...
must_haves:
  ...
  key_links:
    ...
    - from: "hooks/useLearnerProfile.ts"
      ...
      pattern: "bootstrapProfile\\(childId"
---
 ...
 *
 * Usage:
 *   const { profile, isLoading, error } = useLearnerProfile(childId, sessions);
 */

import { useState, useEffect, useCallback, useRef } from 'react';
import { LearnerProfile, TopicMastery, MasteryLevel, getMasteryLevel, StudySession } from '../types';
...
export function useLearnerProfile(
    ...
    const unsubscribe = subscribeToProfile(
      ...
      (data) => {
        ...
        if (
          ...
          bootstrapProfile(childId, sessions).catch(err => {
  ...
  }, [childId, sessions.length, options.skipBootstrap]);
...
```
  ...
  <done>hooks/useLearnerProfile.ts exists with useLearnerProfile hook, auto-bootstrap logic, and getConfidenceMessage utility</done>
</task>
...
6. Hook auto-bootstraps profile when profile is null but sessions exist
7. Bootstrap is fire-and-forget (doesn't block UI)
8. Bootstrap only attempted once per child (tracked via ref)
</verification>
