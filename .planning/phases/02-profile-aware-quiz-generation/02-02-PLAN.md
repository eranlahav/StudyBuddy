...
**IMPORTANT:** To avoid variable shadowing with `topic` (options.topic), extract the session topic at the start of handleAnswer:
...
```typescript
  ...
  const handleAnswer = useCallback((optionIndex: number) => {
    ...
    const questionTopic = currentQ.topic || topic; // 'topic' here is from options destructured above
    ...
    // Update frustration tracking (using questionTopic, not shadowed 'topic')
...
Note: The variable `questionTopic` is used instead of `topic` to avoid shadowing the `topic` from options. Use `questionTopic` throughout handleAnswer for frustration tracking.
