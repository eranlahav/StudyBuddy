...
And extend QuizQuestion type to include optional topic field. Check types.ts - if QuizQuestion doesn't have topic field, add it:
```typescript
// In types.ts QuizQuestion interface (around line 112), add:
export interface QuizQuestion {
  ...
  audioText?: string;
  topic?: string;  // ADD THIS - Topic for tracking (used by adaptive quiz)
}
```
  </action>
  <verify>
...
- [ ] QuizQuestion interface includes optional topic field
