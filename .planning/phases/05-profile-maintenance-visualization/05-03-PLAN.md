...
<objective>
Create the probe scheduler service and extend adaptive quiz service with review mode.
...
Output: `services/probeScheduler.ts` with SM-2 scheduling, extended `adaptiveQuizService.ts` with review mode and probe topic selection.
</objective>
