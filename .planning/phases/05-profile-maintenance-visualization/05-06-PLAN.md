...
must_haves:
  ...
  artifacts:
    ...
    - path: "pages/ChildDetails/AlertNotificationBanner.tsx"
      provides: "Dismissable notification banner component"
      exports: ["AlertNotificationBanner"]
  key_links:
    ...
    - from: "pages/ChildDetails/index.tsx"
      to: "pages/ChildDetails/AlertNotificationBanner.tsx"
      via: "Component import and render"
      pattern: "import.*AlertNotificationBanner.*from"
---
...
Output: Extended profileService with decay wrapper, useLearnerProfile with alert detection, and AlertNotificationBanner component wired into ChildDetails.
</objective>
...
@hooks/useQuizSession.ts  # Plan 03 - review mode state
...
<task type="auto">
  <name>Task 3: Create AlertNotificationBanner and wire into ChildDetails</name>
  <files>pages/ChildDetails/AlertNotificationBanner.tsx, pages/ChildDetails/index.tsx</files>
  <action>
Create the notification banner component and integrate it into ChildDetails:

1. **Create `pages/ChildDetails/AlertNotificationBanner.tsx`:**
```typescript
...
/**
 * Dismissable notification banner for regression alerts
 * Displays at top of ChildDetails when activeNotification is set
 * Auto-dismisses after 8 seconds (handled by useLearnerProfile hook)
 */
export function AlertNotificationBanner({ alert, onDismiss }: AlertNotificationBannerProps) {
    ...
    >
      ...
      <div className="flex-1">
        ...
        {alert.recommendation && (
          <p className="text-amber-700 text-sm mt-1">{alert.recommendation}</p>
        )}
      </div>
      <button
        onClick={onDismiss}
...
2. **Wire into `pages/ChildDetails/index.tsx`:**
...
In the component, get activeNotification and dismissNotification from useLearnerProfile:
```typescript
const {
  profile,
  decayedProfile,
  ...
  error: profileError,
  getConfidenceLevel,
  activeNotification,    // NEW
  dismissNotification    // NEW
} = useLearnerProfile(childId, child);
```

Render the banner at the top of the main content area (after the header, before tabs):
```typescript
...
The banner will:
- Display when activeNotification is set (regression detected)
- Show Hebrew message and recommendation
- Allow manual dismiss via X button
- Auto-dismiss after 8 seconds (via hook timer)
  </action>
  <verify>
Run `npx tsc --noEmit` - no type errors

Manual verification:
- AlertNotificationBanner.tsx exports AlertNotificationBanner component
- ChildDetails imports and renders AlertNotificationBanner
- Banner shows when activeNotification is not null
- Dismiss button calls dismissNotification
  </verify>
  <done>
AlertNotificationBanner component created and wired into ChildDetails.
Regression alerts now display visually (not just in React state).
Banner supports manual and automatic dismissal.
  </done>
</task>
...
4. AlertNotificationBanner:
   - Renders amber warning banner when alert provided
   - Shows message and recommendation
   - Dismiss button functional
   - Accessible (role="alert", aria-live)
5. ChildDetails:
   - Imports and renders AlertNotificationBanner
   - Passes activeNotification and dismissNotification
</verification>
...
- Alerts displayed via AlertNotificationBanner in ChildDetails
- Notification auto-dismisses after 8 seconds
</success_criteria>
